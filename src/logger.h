...
#define EXT_LOGGER_INIT(handle) logger::LogFactory::GetInstacne().SetLogHandle(handle)

#define LOGGER_CALL(handle, level, ...)                                                                   \
    if (handle) {                                                                                         \
        (handle)->Log(level,                                                                              \
                      logger::SourceLocation{__FILE__, __LINE__, static_cast<const char*>(__FUNCTION__)}, \
                      __VA_ARGS__);                                                                       \
    }
...
#define EXT_LOG_TRACE(...) LOG_LOGGER_TRACE(logger::LogFactory::GetInstacne().GetLogHandle(), __VA_ARGS__)
#else
...
#define EXT_LOG_DEBUG(...) LOG_LOGGER_DEBUG(logger::LogFactory::GetInstacne().GetLogHandle(), __VA_ARGS__)
#else
...
#define EXT_LOG_INFO(...) LOG_LOGGER_INFO(logger::LogFactory::GetInstacne().GetLogHandle(), __VA_ARGS__)
#else
...
#define EXT_LOG_WARN(...) LOG_LOGGER_WARN(logger::LogFactory::GetInstacne().GetLogHandle(), __VA_ARGS__)
#else
...
#define EXT_LOG_ERROR(...) LOG_LOGGER_ERROR(logger::LogFactory::GetInstacne().GetLogHandle(), __VA_ARGS__)
#else
...
#define EXT_LOG_CRITICAL(...) LOG_LOGGER_CRITICAL(logger::LogFactory::GetInstacne().GetLogHandle(), __VA_ARGS__)
#else
